...
func Prove(r1cs *cs.R1CS, pk *ProvingKey, witness fr.Vector, opt backend.ProverConfig) (*Proof, error) {
	...
	computeKRS := func() {
		...
		go func() {
			//todo
			res, err := MsmBN254GnarkAdapter(pk.G1.Z, h);
			if err == nil {
				krs2 = res
			}
			chKrs2Done <- err
		...
		//todo
		res, err := MsmBN254GnarkAdapter(pk.G1.K, _wireValues[r1cs.GetNbPublicVariables():]);
		if err != nil {
			chKrsDone <- err
			return
		}
	...
	/*
	TODO; later support G2 as it seems we dont support it just yet
	*/
